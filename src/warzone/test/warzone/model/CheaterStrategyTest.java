package warzone.model;

import org.junit.Test;

/**
 * test class of CheaterStrategy
 */
public class CheaterStrategyTest {

    /**
     * test create order
     */
    @Test
    public void testCreateOrder() {
//        Player l_player = new Player("P1");
//        Country l_country1 = new Country(1,"C1",0,0,null);
//        Country l_country2 = new Country(2,"C2",0,0,null);
//        Country l_country3 = new Country(3,"C2",0,0,null);
//        GameContext.getGameContext().getCountries().put(l_country1.getCountryID(),l_country1);
//        GameContext.getGameContext().getCountries().put(l_country2.getCountryID(),l_country2);
//        GameContext.getGameContext().getCountries().put(l_country3.getCountryID(),l_country3);
//        l_country1.setArmyNumber(5);
//        l_country2.setArmyNumber(3);
//        l_country3.setArmyNumber(3);
//        l_country1.setCountryState(CountryState.Occupied, l_player);
//        l_country2.setCountryState(CountryState.Occupied, null);
//        l_country3.setCountryState(CountryState.Neutral, null);
//        l_player.getConqueredCountries().put(l_country1.getCountryID(), l_country1);
//        l_country1.addNeighbor(l_country2);
//        l_country2.addNeighbor(l_country1);
//        l_country1.addNeighbor(l_country3);
//        l_country3.addNeighbor(l_country1);
//		PlayerStrategy l_strategy=new CheaterStrategy(l_player);
//		Order l_order=l_strategy.createOrder();
//		assert(l_country1.getArmyNumber()==10);
    }

}
